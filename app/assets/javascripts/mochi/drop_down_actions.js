console.log("mochi-dropdowns");
$(() => {
  $(document).on('click', '.mochi--toggle-drop-down', async evt => {
    const $tgt = $(evt.target).is('button') ? $(evt.target) : $(evt.target).parents('button');
    // const $tgt = $(evt.target);
    console.log($tgt);
    const dropdownId = $tgt.data('drop-down-id');
    console.log(dropdownId);
    const $menu = $(document).find(`.mochi--drop-down-menu[data-drop-down-id='${dropdownId}']`)
    console.log($menu);
    if ($menu.is(':visible')) {
      $menu.hide();
    } else {
